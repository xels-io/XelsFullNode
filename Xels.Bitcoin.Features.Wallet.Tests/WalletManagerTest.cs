...
namespace Xels.Bitcoin.Features.Wallet.Tests
{
    public class WalletManagerTest : LogsTestBase, IClassFixture<WalletFixture>
    {
        ...
        public void CreateWalletWithoutPassphraseOrMnemonicCreatesWalletUsingPassword()
        {
            ...
            var walletManager = new WalletManager(this.LoggerFactory.Object, KnownNetworks.XelsMain, chain, new WalletSettings(NodeSettings.Default(this.Network)),
            ...
            Assert.Equal(KnownNetworks.XelsMain, expectedWallet.Network);
            ...
            {
                Assert.Equal(CoinType.Xels, expectedWallet.AccountsRoot.ElementAt(i).CoinType);
            ...
            Assert.Equal(actualWallet.EncryptedSeed, mnemonic.DeriveExtKey(password).PrivateKey.GetEncryptedBitcoinSecret(password, KnownNetworks.XelsMain).ToWif());
            Assert.Equal(expectedWallet.EncryptedSeed, mnemonic.DeriveExtKey(password).PrivateKey.GetEncryptedBitcoinSecret(password, KnownNetworks.XelsMain).ToWif());
        }
        ...
        public void CreateWalletWithPasswordAndPassphraseCreatesWalletUsingPasswordAndPassphrase()
        {
            ...
            Assert.Equal(actualWallet.EncryptedSeed, mnemonic.DeriveExtKey(passphrase).PrivateKey.GetEncryptedBitcoinSecret(password, KnownNetworks.XelsMain).ToWif());
            Assert.Equal(expectedWallet.EncryptedSeed, mnemonic.DeriveExtKey(passphrase).PrivateKey.GetEncryptedBitcoinSecret(password, KnownNetworks.XelsMain).ToWif());
        }
        ...
        public void LoadWalletWithExistingWalletLoadsWalletOntoManager()
        {
            ...
            var walletManager = new WalletManager(this.LoggerFactory.Object, KnownNetworks.XelsMain, new Mock<ChainIndexer>().Object, new WalletSettings(NodeSettings.Default(this.Network)),
                                                dataFolder, new Mock<IWalletFeePolicy>().Object, new Mock<IAsyncProvider>().Object, new NodeLifetime(), DateTimeProvider.Default, new ScriptAddressReader());
        ...
        public void RecoverWalletWithEqualInputAsExistingWalletRecoversWallet()
        {
            ...
            var walletManager = new WalletManager(this.LoggerFactory.Object, KnownNetworks.XelsMain, chainIndexer, new WalletSettings(NodeSettings.Default(this.Network)),
                dataFolder, new Mock<IWalletFeePolicy>().Object, new Mock<IAsyncProvider>().Object, new NodeLifetime(), DateTimeProvider.Default, new ScriptAddressReader());
        ...
        public void LastBlockHeightWithoutWalletsOfCoinTypeReturnsZero()
        {
            var walletManager = new WalletManager(this.LoggerFactory.Object, this.Network, new Mock<ChainIndexer>().Object, new WalletSettings(NodeSettings.Default(this.Network)),
                CreateDataFolder(this), new Mock<IWalletFeePolicy>().Object, new Mock<IAsyncProvider>().Object, new NodeLifetime(), DateTimeProvider.Default, new ScriptAddressReader());
            Wallet wallet = this.walletFixture.GenerateBlankWallet("myWallet", "password");
            wallet.AccountsRoot.ElementAt(0).CoinType = CoinType.Xels;
            walletManager.Wallets.Add(wallet);
        ...
        public void GetSpendableTransactionsReturnsTransactionsGivenBlockHeight()
        {
            ...
            {
                CoinType = CoinType.Xels,
                Accounts = new List<HdAccount>
                {
                    new HdAccount {
                        ExternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(KnownNetworks.XelsMain, 8,9,10),
                        InternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(KnownNetworks.XelsMain, 8,9,10)
                    }
                }
            });

            Wallet wallet2 = this.walletFixture.GenerateBlankWallet("myWallet2", "password");
            wallet2.AccountsRoot.ElementAt(0).CoinType = CoinType.Xels;
            wallet2.AccountsRoot.ElementAt(0).Accounts.Add(new HdAccount
            {
                ExternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(KnownNetworks.XelsMain, 1, 3, 5, 7, 9, 10),
                InternalAddresses = WalletTestsHelpers.CreateUnspentTransactionsOfBlockHeights(KnownNetworks.XelsMain, 2, 4, 6, 8, 9, 10)
            });
        ...
        {
            string result = HdOperations.CreateHdPath((int)CoinType.Xels, 4, true, 3);
        ...
        {
            string result = HdOperations.CreateHdPath((int)CoinType.Xels, 4, false, 3);
        ...
        public void AddNewAccount_via_xpubkey_prevents_adding_an_account_as_an_existing_account_index()
        {
            ...
            var walletManager = this.CreateWalletManager(dataFolder, KnownNetworks.XelsMain);
            var wallet = walletManager.RecoverWallet("wallet1", ExtPubKey.Parse(xelsAccount0ExtPubKey), 0, DateTime.Now.AddHours(-2));

            try
            {
                wallet.AddNewAccount(ExtPubKey.Parse(xelsAccount1ExtPubKey), 0, DateTime.Now.AddHours(-2));
        ...
        public void AddNewAccount_via_xpubkey_prevents_adding_the_same_xpub_key_as_different_account()
        {
            ...
            var addNewAccount = new Action(() => wallet.AddNewAccount(ExtPubKey.Parse(xelsAccount0ExtPubKey), 1, DateTime.Now.AddHours(-2)));
        ...
        {
            DataFolder dataFolder = CreateDataFolder(this);
            var walletManager = this.CreateWalletManager(dataFolder, KnownNetworks.XelsMain, "-defaultwalletname=default");
            walletManager.Start();
        ...
        {
            DataFolder dataFolder = CreateDataFolder(this);
            var walletManager = this.CreateWalletManager(dataFolder, KnownNetworks.XelsMain, "-defaultwalletname=default", "-defaultwalletpassword=default2");
            walletManager.Start();
            Assert.True(walletManager.ContainsWallets);
        ...
        {
            DataFolder dataFolder = CreateDataFolder(this);
            var walletManager = this.CreateWalletManager(dataFolder, KnownNetworks.XelsMain, "-defaultwalletname=default", "-unlockdefaultwallet");
            walletManager.Start();
            Assert.True(walletManager.ContainsWallets);
