...
namespace Xels.Bitcoin.Features.LightWallet.Tests
{
    /// <summary>
    ...
    public class LightWalletFixedFeePolicyTest
    {
        ...
        [Fact]
        public void MinTxFee_OnXelsNetwork_IsXelsDefault()
        {
            ILoggerFactory loggerFactory = new LoggerFactory();
            var nodeSettings = new NodeSettings(KnownNetworks.XelsTest);
            var policy = new LightWalletFixedFeePolicy(loggerFactory, nodeSettings);
            Assert.Equal(new FeeRate(KnownNetworks.XelsTest.FallbackFee), policy.FallbackTxFeeRate);
        }
