...
namespace NBitcoin
{
    ...
    public class ChainIndexer
    {
        ...
        public readonly Dictionary<int, ChainedHeader> blocksByHeight;
        ...
        /// <summary>
        /// TODO: Make this internal when the component moves to Xels.Bitcoin
        /// </summary>
        ...
        public virtual List<ChainedHeader> GetAllHeader()
        {
            int i = 0;
            ChainedHeader chainedHeader = null;
            List<ChainedHeader> lstChainedHeader = new List<ChainedHeader>();
            while (true)
            {
                chainedHeader = this.GetHeader(i);
                if (chainedHeader == null)
                    break;
                lstChainedHeader.Add(chainedHeader);
                i++;
            }
            lstChainedHeader.Reverse();
            this.CountBlock = i + 1;
            return lstChainedHeader;
            //lock (this.lockObject)
            //{
            //    ChainedHeader result;
            //    this.blocksByHeight.TryGetValue(height, out result);
        ...
        public List<ChainedHeader> GetLastNBlocksInfo(int numberOfBlocks)
        {
            int blockAtHeight = this.Tip.Height;
            List<ChainedHeader> lstChainedBlocks = new List<ChainedHeader>();
            for (int i = 0; i < numberOfBlocks; i++)
