...
namespace NBitcoin
{
    ...
    public class ConcurrentChain : ChainBase
    {
        ...
        public List<ChainedHeader> GetAllBlocks()
        {
            int i = 0;
            ChainedHeader block = null;
            List<ChainedHeader> lstChainedBlocks = new List<ChainedHeader>();
            while (true)
            {
                ...
                if (block == null)
                    break;
                lstChainedBlocks.Add(block);
                i++;
            }
            lstChainedBlocks.Reverse();
            return lstChainedBlocks;
        }

        public List<ChainedHeader> GetAllBlocks(int height)
        {
            List<ChainedHeader> lstChainedBlocks = new List<ChainedHeader>();
            for (int i = height; i >= 0; i--)
            {
                lstChainedBlocks.Add(this.GetBlockLocked(i));
            }
            return lstChainedBlocks;
        }
