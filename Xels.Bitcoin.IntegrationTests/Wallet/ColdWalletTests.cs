    ...
    public class ColdWalletTests
    {
        ...
        public void WalletCanMineWithColdWalletCoins()
        {
            using (NodeBuilder builder = NodeBuilder.Create(this))
            {
                ...
                TestHelper.WaitLoop(() => xelsHotStake.CreateRPCClient().GetRawMempool().Length > 0);
                Assert.NotNull(xelsHotStake.CreateRPCClient().GetRawTransaction(transaction1.GetHash(), false));
                TestHelper.WaitLoop(() => xelsHotStake.FullNode.WalletManager().GetSpendableTransactionsInWallet(WalletName).Any());
                ...
                // Broadcast to the other node
                xelsHotStake.FullNode.NodeService<WalletController>().SendTransaction(new SendTransactionRequest(transaction2.ToHex()));
