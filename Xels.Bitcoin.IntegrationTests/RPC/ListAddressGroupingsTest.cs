    ...
    public sealed class ListAddressGroupingsTest
    {
        ...
        public async Task ListAddressGroupingsAsync()
        {
            using (var builder = NodeBuilder.Create(this))
            {
                ...
                this.miner = builder.CreateXelsPowNode(network, agent: "la-1-miner", configParameters: nodeConfig).WithWallet().Start();
                this.receiver = builder.CreateXelsPowNode(network, agent: "la-1-receiver", configParameters: nodeConfig).WithWallet().Start();
