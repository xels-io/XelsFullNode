...
namespace Xels.Bitcoin.IntegrationTests.API
{
    public partial class ApiSpecification : BddSpecification
    {
        ...
        private const string XelsRegTest = "XelsRegTest";
        ...
        private void calling_startstaking()
        {
            ...
            this.xelsPosApiNode.Mnemonic = this.xelsPosApiNode.FullNode.WalletManager().CreateWallet(WalletPassword, WalletName, WalletPassphrase);
        ...
        {
            //NOTE: use legacy xels xpub key format for this one to ensure that works too.
        ...
        private void calling_addnode_connects_two_nodes()
        {
            ...
            TestHelper.WaitLoop(() => TestHelper.AreNodesSynced(this.firstXelsPowApiNode, this.secondXelsPowApiNode));
        }
        ...
        private void status_information_is_returned()
        {
            ...
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Base.BaseFeature");
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Features.Api.ApiFeature");
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Features.BlockStore.BlockStoreFeature");
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Features.Consensus.PowConsensusFeature");
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Features.MemoryPool.MempoolFeature");
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Features.Miner.MiningFeature");
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Features.RPC.RPCFeature");
            statusResponse.EnabledFeatures.Should().Contain("Xels.Bitcoin.Features.Wallet.WalletFeature");
        }
        ...
        private void general_information_about_the_wallet_and_node_is_returned()
        {
            ...
            generalInfoResponse.WalletFilePath.Should().ContainAll(XelsRegTest, $"{WalletName}.wallet.json");
            generalInfoResponse.Network.Name.Should().Be(XelsRegTest);
        ...
        {
            var miningRpcController = this.xelsPosApiNode.FullNode.NodeService<StakingRpcController>();
        ...
        private void SendTransaction(IActionResult transactionResult)
        {
            ...
            this.firstXelsPowApiNode.FullNode.NodeService<WalletController>().SendTransaction(new SendTransactionRequest(walletTransactionModel.Hex));
        }

        private IActionResult BuildTransaction()
        {
            IActionResult transactionResult = this.firstXelsPowApiNode.FullNode.NodeService<WalletController>()
