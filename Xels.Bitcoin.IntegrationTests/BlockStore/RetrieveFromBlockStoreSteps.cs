...
namespace Xels.Bitcoin.IntegrationTests.BlockStore
{
    public partial class RetrieveFromBlockStoreSpecification : BddSpecification
    {
        ...
        {
            this.node = this.builder.CreateXelsPowNode(this.network).WithReadyBlockchainData(Common.ReadyData.ReadyBlockchain.BitcoinRegTest100Miner).Start();
        }

        private void a_pow_node_to_transact_with()
        {
            this.transactionNode = this.builder.CreateXelsPowNode(this.network).WithReadyBlockchainData(Common.ReadyData.ReadyBlockchain.BitcoinRegTest100Miner).Start();
