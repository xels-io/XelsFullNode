namespace NBitcoin.Tests
{
    public class pos_ProtocolTests
    {
        ...
        public void CanParseMessages()
        {
            ...
            {
                var message = XelsNetworks.XelsMain.ParseMessage(TestUtils.ParseHex(test.Message), test.Version);
        ...
        public void CanCancelConnection()
        {
            ...
            using (var builder = NodeBuilderXels.Create())
            {
                ...
                try
                {
                    var client = Node.Connect(XelsNetworks.XelsMain, "127.0.0.1:" + node.ProtocolPort.ToString(), new NodeConnectionParameters()
                    {
        ...
        public void CanGetTransactionsFromMemPool()
        {
            ...
            using (var builder = NodeBuilderXels.Create())
            {
                ...
                //node.Start();
                //node.Generate(101);
                //node.CreateRPCClient().SendToAddress(new Key().PubKey.GetAddress(XelsNetworks.XelsMain), Money.Coins(1.0m));
        ...
        public void CanDownloadBlock()
        {
            ...
            using (var builder = NodeBuilderXels.Create())
            {
                ...
                {
                    Hash = XelsNetworks.XelsMain.GenesisHash,
