...
namespace Xels.Bitcoin.IntegrationTests.Common.EnvironmentMockUpHelpers
{
    public class NodeBuilder : IDisposable
    {
        ...
        private static string GetXelsXPath(string version)
        {
            string path;

            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
                path = $"../../../../External libs/XelsX/{version}/Windows/xelsd.exe";
            else if (RuntimeInformation.IsOSPlatform(OSPlatform.Linux))
                path = $"../../../../External libs/XelsX/{version}/Linux/xelsd";
            else
                path = $"../../../../External libs/XelsX/{version}/OSX/xelsd";
        ...
        {
            var node = new CoreNode(new XelsBitcoinPowRunner(cloneNode.FullNode.Settings.DataFolder.RootPath, cloneNode.FullNode.Network, agent), this.ConfigParameters, "bitcoin.conf");
            this.Nodes.Add(node);
